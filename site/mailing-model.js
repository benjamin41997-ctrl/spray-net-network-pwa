...
export function validateRecipient(r) {
  ...
  if (!r || fields.some(k => !Object.hasOwn(r,k)) || Object.keys(r).some(k => !fields.includes(k) && !['prospectNotes','exterior','addressSourceType','prospect','property'].includes(k))) throw Error('Invalid mailing recipient fields.');
  if (Object.hasOwn(r,'prospect')) validateProspect(r.prospect);
  if (Object.hasOwn(r,'property')) validateProperty(r.property);
...
export function mergeMailingCatalog(recipients,companies){
  ...
  return [...recipients,...companies.filter(c=>!seen.has(c.id)).map(c=>({id:c.id,name:c.name,category:c.category,attention:'',address1:'',address2:'',city:c.city||'',state:c.state||'',zip:c.zip||'',country:'US',source:'',reviewedOn:'',status:'needs_review'}))].map(r=>{
    const v=byId.get(r.id)?.visit;
    if(r.prospect?.visit||!v?.checkedAt||!v.sources?.length)return r;
    const value=({walk_in:'walk_in',appointment_only:'appointment',home_based:'home_based'})[v.status]||'unknown';
    return {...r,prospect:{...r.prospect,visit:{value,reason:v.summary,source:v.sources[0],reviewedOn:v.checkedAt}}};
  });
}
...
export const defaultFilters = () => ({categories:['dentist','school'],cities:[],zips:'',search:'',excludeDays:0,onePerAddress:false,excluded:[],area:'',exterior:'',sourceType:'',opportunity:'',access:'',evidence:'',visit:''});
export function validateFilters(f) {
  for(const k of Object.keys(ratingOptions))if(f?.[k]!==undefined&&f[k]!==''&&!Object.hasOwn(ratingOptions[k],f[k]))throw Error('Invalid prospect rating filter.');
...
export function selectRecipients(catalog, filters, suppressed=[], lastMailed=()=>null, today, findingsFor=()=>({})) {
  ...
  const candidates = catalog.filter(r=>Object.keys(ratingOptions).every(k=>!filters[k]||prospectRatings(r,findingsFor(r.id))[k].value===filters[k])&&(!filters.sourceType||r.addressSourceType===filters.sourceType)&&filters.categories.includes(r.category) && (!filters.area||mailingArea(r)===filters.area) && (!filters.exterior||approvalRoute(r,findingsFor(r.id))===filters.exterior) && (!filters.cities.length||filters.cities.includes(r.city)) && (!zips.length||zips.includes(r.zip.slice(0,5))) && (!filters.search||[r.name,r.city,r.address1,r.address2].join(' ').toLowerCase().includes(filters.search.toLowerCase()))).sort((a,b)=>a.name.localeCompare(b.name)||a.id-b.id);
